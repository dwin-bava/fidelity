...
function FacilityEnquiry() {
  ...
  async function onOkPress() {
    ...
    await axios
      .post("http://localhost:2030/loan-general-enquiry", {
        name: customerName,
  ...
  useEffect(()=>{
      axios.get("http://localhost:2030/get-loan-general-enquiry").then(function(response){
          if(response.status === 200){
          console.log(response.data)
...
//   return(
//     arr.push([item["principal_account"],item["name"],null,item['tenor'],item['effective_date'],item['expiry_date'],item['interest_rate'],item['amount_granted'],item["total_balance"],item["accrued_int"],item["accrued_penalty"],item['class_code'],null, <div style={{padding: '10px', backgroundColor:"green", borderRadius: '10px', textAlign: 'center'}} onClick={() => {handleShow(), setUniqueTuple({principal_account : loanData[key].principal_account , facility_no: loanData[key].facility_no})}}><FiChevronRight color="white" size={23} /></div> ])
//   )
//  })
      ...
      <div style={{ display: "flex", flex: 1 }}>
        ...
        <div
          style={{
            ...
            marginRight: "10px",
            // boxShadow: "rgba(0, 0, 0, 0.24) 0px 3px 8px",
            boxShadow: "9px 2px 16px 1px rgba(166,164,164,0.75)",
